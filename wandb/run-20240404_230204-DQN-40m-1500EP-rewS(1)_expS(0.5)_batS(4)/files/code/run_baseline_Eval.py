...
def make_model(algorithm):
    if algorithm == ("PPO"):
        ...
        model = DQN('CnnPolicy', env, verbose=1, gamma=gamma, tensorboard_log=sess_path)
...
if __name__ == "__main__":
    ...
    env_config = {
            ...
            'gb_path': 'PokemonRed.gb', 'debug': False, 'reward_scale': reward_scale, 'explore_weight': explore_weight, 'battle_weight': battle_weight,
    ...
    if use_wandb_logging:
        ...
        run = wandb.init(
            project="FYP-RL-DATA",
            id=f"{algorithm}-{mil_timestep}m-{episode_length_per_cpu}EP-rewS({reward_scale})_expS({explore_weight})_batS({battle_weight})", #PPO-40m-1500EP-rewS:1.0_expS:0.5_batS:4
            config=env_config,
